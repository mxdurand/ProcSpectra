### Get time for spectra measurement, but for dark files only (useful when dealing with high-speed acquisition)
getMayaTimeDark <- function(subFolder, Folder, User = "Localadmin_durandma", Year = 2020)
{
  ...
  for (iDark in darkFiles)
  {
    ...
    firstFile <- list.files(pathToFiles)[grep(iDark, list.files(pathToFiles))[1]]
